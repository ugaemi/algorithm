...
def fatigue(k: int, dungeons: List[List[int]]) -> int:
    max_dungeons = 0
    for dungeons in permutations(dungeons):
        count, f = 0, k
        for min_fatigue, consumption_fatigue in dungeons:
            if f >= min_fatigue and f >= consumption_fatigue:
                count += 1
                f -= consumption_fatigue
...
class Test(unittest.TestCase):
    def test(self):
        self.assertEqual(fatigue(80, [[80, 20], [50, 40], [30, 10]]), 3)
